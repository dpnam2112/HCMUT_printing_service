...
def pre_check(document_path, pages_print, check_info, username, date_send, filename, num_copies, side):
    ...
    if pages_print != 'all':
        ...
        if (min(all_pages) < 1 or max(all_pages) > total_pages):
            check_info.append("Số trang bạn nhập nhỏ hơn 1 hoặc lớn hơn số trang tài liệu bạn cung cấp")
            return False
    ...
    #kiem tra so trang
    if not settings.FRONTEND_DEV and username.is_superuser:
        p = CampusUser.objects.filter(base_user = username)
        print(p[0].page_balance)
