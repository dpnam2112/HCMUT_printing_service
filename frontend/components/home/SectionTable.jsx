  ...
  if (status == "In Thành công")
    ...
    return (
      <div className="flex items-center gap-2 w-full h-full">
        ...
        <span className="text-sm font-medium text-red-400">In bị lỗi</span>
      </div>
    );
    ...
    return (
      <div className="flex items-center gap-2 w-full h-full">
        ...
        <span className="text-sm font-medium text-[#FFA500]">Đang được in</span>
      </div>
    );
...
function RenderTableDataRow({ data }) {
  if (data)
    return (
      <Table.Row>
        <Table.Cell>
          <RenderActivityStatus status={data.activity_status}>
            {" "}
    ...
    <div className="flex flex-col mt-16 mx-40 max-w-[1700px] min-h-[400px] px-10 py-14 rounded border">
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center justify-center gap-5 w-1/2">
            ...
            <Button className="cursor-pointer">
              <Link href={"/admin"}>Xem lịch sử in</Link>
            </Button>
            <Button className="cursor-pointer">
              <Link href={"/pricing"}>Mua thêm giấy in</Link>
            </Button>
          </div>
      ...
      <div className="font-medium text-sm rounded border mt-5">
        <Table.Root className="overflow-auto">
            ...
            <Table.Row>
              <Table.ColumnHeaderCell>
                <div
                  ...
                  onClick={() => setSort("activity_status")}
                >
              ...
              <Table.ColumnHeaderCell>
                <div
                  className="flex items-center gap-1 cursor-pointer"
                  onClick={() => setSort("number_of_page")}
                >
                  <span>Số trang</span>
                  <CaretSortIcon className="w-4 h-4" />
                </div>
          ...
          <Table.Body>
            <RenderTableDataRow data={final_Data[currentPage * 5]}>
              {" "}
            </RenderTableDataRow>
            <RenderTableDataRow data={final_Data[currentPage * 5 + 1]}>
              {" "}
            </RenderTableDataRow>
            <RenderTableDataRow data={final_Data[currentPage * 5 + 2]}>
              {" "}
            </RenderTableDataRow>
            <RenderTableDataRow data={final_Data[currentPage * 5 + 3]}>
              {" "}
            </RenderTableDataRow>
            <RenderTableDataRow data={final_Data[currentPage * 5 + 4]}>
              {" "}
            </RenderTableDataRow>
          </Table.Body>
        </Table.Root>
      ...
      <div className="flex items-center justify-end mt-5 w-full h-fit">
        <div className="flex items-center justify-center gap-5">
          <span className="font-medium text-base">
            Trang {currentPage + 1} trên{" "}
            {Math.ceil(Object.keys(final_Data).length / 5)}
          </span>
          <div className="flex items-center gap-5">
            ...
            <div className="flex items-center gap-2">
              <IconButton
                ...
                disabled={
                  currentPage >
                  Math.floor(Object.keys(final_Data).length / 5) - 1
                }
              >
              ...
              <IconButton
                onClick={() =>
                  setcurrentPage(
                    Math.ceil(Object.keys(final_Data).length / 5 - 1)
                  )
                }
                disabled={
                  currentPage >
