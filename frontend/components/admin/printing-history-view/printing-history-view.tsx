...
const PrintingTableView = () => {
  ...
  useState<HistoryDataObject[]>(mData);
const [tableListRendered, setTableListRendered] =
  ...
  useEffect(() => {
    ...
    setTableListRendered(getFilteredTimeList(getFilteredList(newHistoryListRendered)));
  }, [sortConfig]);
  ...
  const RenderSortableHeaderCell :FC<TableHeaderProps> = ({
    ...
    return (
        ...
        >
          ...
          {sortConfig === ASConfig ? <ArrowUpIcon /> : sortConfig === DESConfig ? <ArrowDownIcon /> : <CaretSortIcon />}
        </div>
    );
  };
